...
int main( void )
{
   ...
   // The ADC has different voltage reference options, controlled by the REFSEL
   // bits in the
   // REFCTRL register. Here the internal reference is selected
   ADC_Reference_Config(&ADCB, ADC_REFSEL_VCC_gc);

   // The clock into the ADC decide the maximum sample rate and the conversion
   // time, and
   // this is controlled by the PRESCALER bits in the PRESCALER register. Here,
   // the
   ...
   // In the while (1) loop, a conversion is started on CH0 and the 8 MSB of the
   // result is
